...
--VitroMod.Triggers.sendToClient = VitroMod.Triggers.sendToClient
...
function ENT:StartTouch(entity)
	...
	--if entity:GetClass() ~= 'player' and entity:GetClass() ~= 'gmod_train_wheels' or entity:GetClass() == 'prop_door_rotating' then return end
	--RunConsoleCommand('say',entity:GetClass()..' '..entity:GetName()..' Start touch'..' '..self:GetName()..' '..self.num)
	--self.cnt = self.cnt + 1
	--if rcNames and VitroMod.Triggers.Count[self:GetName()] == 0 then SendRCInfo(nil,self,1) end
	if VitroMod.Triggers.Count[self:GetName()] == 0 then SendRCInfo(nil,self,1) end
	self.occ = true
...
function ENT:EndTouch(entity)
	...
	if entity:GetClass() ~= 'gmod_train_wheels' then return false end
	--if entity:GetClass() ~= 'player' and entity:GetClass() ~= 'gmod_train_wheels' or entity:GetClass() == 'prop_door_rotating' then return end
	--RunConsoleCommand('say',entity:GetClass()..' '..entity:GetName()..' End touch'..' '..self:GetName()..' '..self.num)
	--self.cnt = self.cnt - 1
	--if(self.cnt == 0) then self.occ = false end
	
	--for k,v in pairs(VitroMod.Triggers.Entities[self:GetName()]) do
		--if v.occ then return end
	--end
